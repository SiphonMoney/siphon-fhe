...
fn duration_div_f64(duration: Duration, n: f64) -> Duration {
    // PATCH (siphon): on coarse/non-monotonic clocks (Docker on Apple-Silicon), the FFT
    // auto-tuning timing math can yield a non-finite or negative value, making
    // Duration::from_secs_f64 panic ("cannot convert float seconds to Duration: value is
    // negative"). Clamp to a finite, non-negative number of seconds.
    let secs = duration.as_secs_f64() / n;
