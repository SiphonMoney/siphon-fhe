...
def debug_eval():
    ...
    # DEBUG: persist the exact wasm-generated bytes so a native Rust test can reproduce.
    try:
        with open('/tmp/st_client.hex', 'w') as f: f.write(data['client_key'])
        with open('/tmp/st_server.hex', 'w') as f: f.write(data['server_key'])
        with open('/tmp/st_ct.hex', 'w') as f: f.write(data['encrypted_lower_bound'])
    except Exception as _e:
        print(f"[debugEval] dump failed: {_e}")
